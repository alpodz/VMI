...
namespace QTOutgoingGMail
{
    
    public class QTOutgoingGMail
    {
        GmailService service;

        [FunctionName("Function1")]
        public async Task Run([QueueTrigger("outgoing-emails", Connection = "AzureWebJobsStorage")] string myQueueItem, ILogger log)
        {
            ...
            service = await CreateGmailService(clientSecretsPath);

        }
    

            private async static Task<GmailService> CreateGmailService(string clientSecretsPath, IDataStore dataStore)
            {
                ...
                using (var stream = new System.IO.FileStream(clientSecretsPath, System.IO.FileMode.Open, System.IO.FileAccess.Read))
                {
                    credential2 = await GoogleWebAuthorizationBroker.AuthorizeAsync(
                        ...
                        "vendormanaged@gmail.com", CancellationToken.None, dataStore);
                }
